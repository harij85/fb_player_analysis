# v5_mvp.py (Formerly v5.4.4_mvp.py - Possession Debug)
import cv2
import numpy as np
...
def get_distance_sq(p1: Optional[Tuple[int, int]], p2: Optional[Tuple[int, int]]) -> float:
    # print(f"DEBUG get_distance_sq: p1={p1} (type {type(p1)}), p2={p2} (type {type(p2)})") # DEBUG
    if p1 is None or p2 is None:
        # print(f"DEBUG get_distance_sq: Returning float('inf') because one input is None.") # DEBUG
        return float('inf')
    # Ensure p1 and p2 are tuples of numbers if not None
    if not (isinstance(p1, tuple) and len(p1) == 2 and isinstance(p1[0], (int, float)) and isinstance(p1[1], (int, float))):
        ...
        return float('inf') # Treat as error / infinite distance
    ...
    # print(f"DEBUG get_distance_sq: Calculated dist_sq = {dist_sq}") # DEBUG
    return dist_sq
...
app = FastAPI(title="Football AI API - v5_mvp (Possession Debug)")
...
async def load_models_and_setup():
    ...
    print("Attempting to load models and setup (v5_mvp Possession Debug)...")
    try:
    ...
    print("--- Startup complete (v5_mvp Possession Debug) ---")
...
def process_frame_for_analysis(
        ...
        for p_draw_info in current_frame_player_tracking_info_for_drawing.values():
            ...
            if smoothed_kpts_for_drawing:
                ...
                for i_skel,j_skel in SKELETON_CONNECTIONS:
                    ...
                    if kp1_info and kp2_info and kp1_info.get('xy') and kp2_info.get('xy'):
                        cv2.line(annotated_frame,kp1_info['xy'],kp2_info['xy'],p_color,2)
                for kp_name_draw, kp_data_draw in smoothed_kpts_for_drawing.items():
                    if kp_data_draw.get('xy'):
                        cv2.circle(annotated_frame, kp_data_draw['xy'], 3, p_color, -1)

    possessor_final_id: Optional[str] = None
    if ball_result_for_storage and ball_result_for_storage.center:
        ball_center_smoothed=ball_result_for_storage.center
        ...
        for player_with_pose in players_with_pose_for_storage:
            ...
            for kp_name in relevant_kpt_names:
                if kp_name in smoothed_kpts and smoothed_kpts[kp_name].get('xy') is not None: # Ensure 'xy' is not None
                    part_xy = smoothed_kpts[kp_name]['xy']
                    
                    if debug_print: # Added more detailed print for possession check
                        print(f"      Possession check: Player {player_with_pose.player_id}, KPT: {kp_name}, part_xy: {part_xy} (type {type(part_xy)}), ball_center: {ball_center_smoothed} (type {type(ball_center_smoothed)})")

                    dist_sq = get_distance_sq(part_xy, ball_center_smoothed)
                    
                    if debug_print:
                        print(f"        dist_sq: {dist_sq} (type {type(dist_sq)}), closest_player_part_dist_sq: {closest_player_part_dist_sq} (type {type(closest_player_part_dist_sq)})")
                    ...
                    if isinstance(dist_sq, (int, float)) and isinstance(closest_player_part_dist_sq, (int, float)):
                        if dist_sq < closest_player_part_dist_sq:
                            closest_player_part_dist_sq=dist_sq
                    elif debug_print:
                         print(f"        SKIPPING COMPARISON: dist_sq or closest_player_part_dist_sq is not a number.")
                elif debug_print:
                    print(f"      Possession check: Player {player_with_pose.player_id}, KPT: {kp_name} - SKIPPED (no xy or kpt not found)")
            ...
            if isinstance(closest_player_part_dist_sq, (int, float)) and closest_player_part_dist_sq < POSSESSION_THRESHOLD_SQ:
                if closest_player_part_dist_sq < min_ball_dist_sq:
                    ...
                    possessor_final_id = player_with_pose.player_id
            elif debug_print and not isinstance(closest_player_part_dist_sq, (int,float)):
                 print(f"      Possession check for {player_with_pose.player_id}: closest_player_part_dist_sq was not a number: {closest_player_part_dist_sq}")
    ...
    return FrameAnalysisResult(
        frame_number_processed=frame_number_processed,
        video_filename_processed=video_filename_processed,
        players=players_with_pose_for_storage,
        ball=ball_result_for_storage,
...
async def analyze_frame_endpoint(
    ...
    print(f"Req-{req_id_suffix}: Frame {frame_number}, Vid '{video_filename}'. Save: {save_to_json_dir is not None}. ReturnImg: {return_annotated_image}")
    ...
    try:
        analysis_output: FrameAnalysisResult = process_frame_for_analysis(
            frame_cv,
            frame_number_processed=frame_number,
            video_filename_processed=video_filename,
        ...
        if save_to_json_dir:
            try:
                ...
                dict_to_save = storage_data.model_dump() # Pydantic V2
...
if __name__ == "__main__":
    import uvicorn
    print("Starting uvicorn server for v5_mvp.py (Possession Debug)...")
    uvicorn.run("v5_mvp:app", host="0.0.0.0", port=8000, workers=1)